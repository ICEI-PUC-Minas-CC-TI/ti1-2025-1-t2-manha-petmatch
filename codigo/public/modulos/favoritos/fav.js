...
async function fetchPets(search) {
  try {
    ...
    const favoritePetIds = favoritePets.map(fav => fav.props.petId);
    const favoritedPets = pets.filter(pet => favoritePetIds.includes(pet.id));

    petsList = favoritedPets;

    drawElements(favoritedPets, favoritePets);
  } catch (err) {
    console.error(err);
  }
}
